namespace Telerik.Examples.Mvc.Areas.SchedulerDragAndDrop.Models
{
    ...
    {
        private SchedulerDragAndDropEntities db;

        public SchedulerMeetingService(SchedulerDragAndDropEntities context)
        {
            db = context;
        }

        public SchedulerMeetingService()
            : this(new SchedulerDragAndDropEntities())
        {
        }
        ...
        public virtual void Insert(MeetingViewModel meeting, ModelStateDictionary modelState)
        {
            if (ValidateModel(meeting, modelState))
            {
                ...
                {
                    entity.MeetingAttendees.Add(new SchedulerDragAndDropMeetingAttendee
                    {
        ...
        public virtual void Update(MeetingViewModel meeting, ModelStateDictionary modelState)
        {
            if (ValidateModel(meeting, modelState))
            {
                ...
                if (meeting.Attendees != null)
                {
                    foreach (var attendeeId in meeting.Attendees)
                    {
                        var meetingAttendee = new SchedulerDragAndDropMeetingAttendee
                        {
        ...
        public virtual void Delete(MeetingViewModel meeting, ModelStateDictionary modelState)
        {
            ...
            var attendees = meeting.Attendees.Select(attendee => new SchedulerDragAndDropMeetingAttendee
            {
                AttendeeID = attendee,
                MeetingID = entity.MeetingID
            });
