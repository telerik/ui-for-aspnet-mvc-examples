namespace Telerik.Examples.Mvc.Areas.SchedulerEditingCustomEditor.Models
{
    ...
    {
        private SchedulerEditingCustomEditorEntities db;

        public SchedulerMeetingService(SchedulerEditingCustomEditorEntities context)
        {
            db = context;
        }

        public SchedulerMeetingService()
            : this(new SchedulerEditingCustomEditorEntities())
        {
        }
        ...
        public virtual void Insert(MeetingViewModel meeting, ModelStateDictionary modelState)
        {
            if (ValidateModel(meeting, modelState))
            {
                ...
                {
                    entity.MeetingAttendees.Add(new SchedulerEditingCustomEditorMeetingAttendee
                    {
        ...
        public virtual void Update(MeetingViewModel meeting, ModelStateDictionary modelState)
        {
            if (ValidateModel(meeting, modelState))
            {
                ...
                if (meeting.Attendees != null)
                {
                    foreach (var attendeeId in meeting.Attendees)
                    {
                        var meetingAttendee = new SchedulerEditingCustomEditorMeetingAttendee
                        {
        ...
        public virtual void Delete(MeetingViewModel meeting, ModelStateDictionary modelState)
        {
            ...
            var attendees = meeting.Attendees.Select(attendee => new SchedulerEditingCustomEditorMeetingAttendee
            {
                AttendeeID = attendee,
                MeetingID = entity.MeetingID
            });
